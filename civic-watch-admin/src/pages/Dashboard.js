...
const DashboardPage = ({ userClaims }) => {
  ...
  useEffect(() => {
    if (!userClaims.zone) return;

    const todayStart = new Date();
    todayStart.setHours(0, 0, 0, 0);
    const todayStartTimestamp = Timestamp.fromDate(todayStart);
    
    const qToday = query(collection(db, 'issues'), where('issueZone', '==', userClaims.zone), where('status', '==', 'Submitted'), where('timestamp', '>=', todayStartTimestamp));
    const unsubToday = onSnapshot(qToday, (snapshot) => {
      setTodaysIssues(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
    });

    const qActive = query(collection(db, 'issues'), where('issueZone', '==', userClaims.zone));
    const unsubActive = onSnapshot(qActive, (snapshot) => {
      setStats(prev => ({ ...prev, active: snapshot.size }));
    });

    const qResolved = query(collection(db, 'resolved_issues'), where('issueZone', '==', userClaims.zone));
